...
%  for alpha = [0.1 0.2 0.3 0.4] (variable viscosity) at omega = 0.02 and plots
...
P.alpha = 0.3;   % variable-viscosity parameter (overwritten by alphaVec below)
...
cols  = lines(numel(alphaVec));
opts  = bvpset('RelTol',1e-6,'AbsTol',1e-8,'NMax',20000);
sol   = cell(size(alphaVec));
ok    = false(size(alphaVec));
...
for i = 1:numel(alphaVec)
    P.alpha = alphaVec(i);
    try
        % previous alpha's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('alpha = %g: %s', P.alpha, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next alpha from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('alpha = %g: residual %.2e -- not converged', P.alpha, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('alpha = %4.2f :  f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            P.alpha, s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$\\alpha = %g$', alphaVec(i)));
    end
