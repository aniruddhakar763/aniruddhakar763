...
%  for omega = [0.01 0.02 0.04 0.06] (nanoparticle volume fraction) and plots
%  f, f', theta, phi in figures 1-4.
%
%  omega enters through L1..L5, the Brinkman viscosity factor (1-omega)^-2.5
%  and the electro-osmotic slip conditions, so all of them are recomputed for
%  every omega.
clear; clc; close all;
...
%% ---------------- Solve for each omega ----------------
eta   = linspace(0,1,801);
cols  = lines(numel(omegaVec));
opts  = bvpset('RelTol',1e-6,'AbsTol',1e-8,'NMax',20000);
sol   = cell(size(omegaVec));
ok    = false(size(omegaVec));
yinit = @(x) [P.St*x; P.St; 0; 0; 0.5*(1-x); -0.5; x; 1];
s     = bvpinit(linspace(0,1,101), yinit);

for i = 1:numel(omegaVec)
    P = setOmega(P, omegaVec(i), TP);
    fprintf('omega = %4.2f :  L1=%.4f  L2=%.4f  L3=%.4f  L4=%.4f  L5=%.4f\n', ...
            P.omega, P.L1, P.L2, P.L3, P.L4, P.L5);
    try
        % previous omega's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('omega = %g: %s', P.omega, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next omega from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('omega = %g: residual %.2e -- not converged', P.omega, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('              f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$\\omega = %g$', omegaVec(i)));
    end
