...
%  for Bi = [0.5 1 1.5 2] (Biot number) at omega = 0.02 and plots
...
P.Bi    = 1.0;   % Biot number (overwritten by BiVec below)
...
for i = 1:numel(BiVec)
    P.Bi = BiVec(i);
    try
        % previous Bi's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('Bi = %g: %s', P.Bi, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next Bi from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('Bi = %g: residual %.2e -- not converged', P.Bi, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('Bi = %4.1f :  f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            P.Bi, s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$Bi = %g$', BiVec(i)));
    end
