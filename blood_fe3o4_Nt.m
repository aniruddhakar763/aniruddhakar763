...
%  for Nt = [0.1 0.2 0.3 0.4] (thermophoresis) at omega = 0.02 and plots
...
P.Nt    = 0.3;   % thermophoresis (overwritten by NtVec below)
...
for i = 1:numel(NtVec)
    P.Nt = NtVec(i);
    try
        % previous Nt's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('Nt = %g: %s', P.Nt, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next Nt from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('Nt = %g: residual %.2e -- not converged', P.Nt, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('Nt = %4.2f :  f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            P.Nt, s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$Nt = %g$', NtVec(i)));
    end
