...
%  for gam = [0.5 1 1.5 2] (Joule heating) at omega = 0.02 and plots
...
P.gam   = 0.5;   % Joule heating (overwritten by gamVec below)
...
cols  = lines(numel(gamVec));
opts  = bvpset('RelTol',1e-6,'AbsTol',1e-8,'NMax',20000);
sol   = cell(size(gamVec));
ok    = false(size(gamVec));
yinit = @(x) [P.St*x; P.St; 0; 0; 0.5*(1-x); -0.5; x; 1];
s     = bvpinit(linspace(0,1,101), yinit);

for i = 1:numel(gamVec)
    P.gam = gamVec(i);
    try
        % previous gam's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('gam = %g: %s', P.gam, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next gam from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('gam = %g: residual %.2e -- not converged', P.gam, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('gam = %4.1f :  f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            P.gam, s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$\\gamma = %g$', gamVec(i)));
    end
