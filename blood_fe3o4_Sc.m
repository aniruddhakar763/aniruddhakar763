...
%  for Sc = [5 10 15 20] (Schmidt number) at omega = 0.02 and plots
...
P.Sc    = 10.0;  % Schmidt number (overwritten by ScVec below)
...
for i = 1:numel(ScVec)
    P.Sc = ScVec(i);
    try
        % previous Sc's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('Sc = %g: %s', P.Sc, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next Sc from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('Sc = %g: residual %.2e -- not converged', P.Sc, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('Sc = %4.1f :  f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            P.Sc, s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$Sc = %g$', ScVec(i)));
    end
