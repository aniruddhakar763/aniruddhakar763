...
%  for Nb = [0.1 0.2 0.3 0.4] (Brownian motion) at omega = 0.02 and plots
...
for i = 1:numel(NbVec)
    P.Nb = NbVec(i);
    try
        % previous Nb's solution is the initial guess for the next one
        s = bvp4c(@(x,y) odefun(x,y,P), @(ya,yb) bcfun(ya,yb,P), s, opts);
    catch ME
        warning('Nb = %g: %s', P.Nb, ME.message);
        s = bvpinit(linspace(0,1,101), yinit);   % restart the next Nb from the simple guess
        continue
    end
    if s.stats.maxres > opts.RelTol
        warning('Nb = %g: residual %.2e -- not converged', P.Nb, s.stats.maxres);
        s = bvpinit(linspace(0,1,101), yinit);
        continue
    end
    sol{i} = s;  ok(i) = true;
    fprintf('Nb = %4.2f :  f''''(0) = %10.6f   theta(0) = %8.5f   phi''(0) = %8.5f   (mesh %d pts)\n', ...
            P.Nb, s.y(3,1), s.y(5,1), s.y(8,1), numel(s.x));
end
...
for k = 1:4
    ...
    for i = find(ok)
        ...
        plot(eta, Y(rows(k),:), 'LineWidth', 1.8, 'Color', cols(i,:), ...
             'DisplayName', sprintf('$Nb = %g$', NbVec(i)));
    end
